...
function remainingPinnedTestimonialsMessage(count: number) {
  if (count > 1)
    return `You have ${count} testimonials left to pin`;
  if (count === 1)
    return `You have only 1 testimonial left to pin`;
...
export default function TestimonialPin() {
  ...
  const remainingPinnedTestimonials = useQuery(api.testimonials.remainingPinTestimonials, {
  ...
  async function handlePin() {
    try {
      const remain = isPinned ? (remainingPinnedTestimonials || 0) + 1 : (remainingPinnedTestimonials || 0) - 1;
    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger>
          ...
          disabled={(!isPinned && (remainingPinnedTestimonials || 0) <= 0) || isPending}>
          {isPinned ? <PinOff /> : <Pin />}
            <span className="sr-only sm:not-sr-only">{isPinned ? "Unpin" : "Pin"}</span>
          </Button>
        </TooltipTrigger>
        {!isPending && !isPinned && remainingPinnedTestimonials !== undefined && <TooltipContent side="bottom">
